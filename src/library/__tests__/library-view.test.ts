...
// library-view: live OPDS debounce + CJK-as-enough search; header next paging;
// search rel=next auto-stream; in-flight browse vs later search race; multi-WebDAV
// catalogs; local WebDAV catalog filter; no-flash 正在加载 on painted catalog;
// later-browse retry after painted covers; null title/comic metadata wall;
// hidden workspace travel control; missing progress as not-started; first comic
// page without 0%; mobile continue-banner/tab-bar spacing; phone manage dialog
// width; IME overlay lift; mobile sources-tab catalog open/return; mobile catalog
// folders + source-named search; mobile detail sheet close/backdrop/Escape;
// mobile cache-limit Escape consume; phone shelf filters without custom/smart
// names; independent Groups entry; Groups sheet custom-group wall; portaled
// 48px Groups list; Groups sheet keeps current filter; deleted group returns
// to All; Groups entry hidden off shelf tab; tab bar without mobile flags;
// tab/chip promotion beyond 760px.
...
describe('LibraryView remove from library', () => {
  ...
  it('hides 移出书库 on remote and navigation catalog menus and details', async () => {
    ...
    const remoteMenu = await openItemMenu(host, 'item-1');
