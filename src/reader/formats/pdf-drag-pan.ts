 ...
 * 锚点；捏合期 touch-action 恒 none，结束按溢出重新收敛。双击（280ms/36px
 * 窗口，pdfIsDoubleTap）在适宽与 2× 档间切换并锚定双击点；窗口内单击被暂扣
 * ≤280ms，无第二击则原样放行——chrome 切换/笔记/划选工具栏等现有点按链不被
 * 双触发。手势结束不额外跳档，落档由 pdf.ts 的 scalechanging→syncScale 吸档
 * 回环负责。
 *
...
export function bindPdfDragPan(
  ...
  /** 超时无第二击：把暂扣的单击原样重派到原目标，点按链 ≤280ms 迟滞放行。 */
  ...
  const onPointerDown = (event: Event): void => {
    ...
    if (activePointers.size >= 2) {
      ...
      if (scaleBinding !== undefined && pinch === null) {
        ...
        if (distance > 0) {
          pinch = { startDistance: distance, startScale: scaleBinding.getCurrentScale() };
  ...
  const onPointerUp = (event: Event): void => {
    ...
    if (pinch !== null && activePointers.size < 2) {
      endPinch();
    }
    let panned = false;
    if (start !== null && pointerId === start.id) {
